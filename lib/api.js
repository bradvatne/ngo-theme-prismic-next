import Prismic from "prismic-javascript";

const REPOSITORY = process.env.PRISMIC_REPOSITORY_NAME;
const REF_API_URL = `https://${REPOSITORY}.prismic.io/api/v2`;
const GRAPHQL_API_URL = `https://${REPOSITORY}.prismic.io/graphql`;
export const API_URL = "https://ngo-next-theme.cdn.prismic.io/api/v2";
export const API_TOKEN = process.env.PRISMIC_API_TOKEN;
export const API_LOCALE = process.env.PRISMIC_REPOSITORY_LOCALE;

export const PrismicClient = Prismic.client(REF_API_URL, {
  accessToken: API_TOKEN,
});

async function fetchAPI(query, { previewData, variables } = {}) {
  const prismicAPI = await PrismicClient.getApi();
  const res = await fetch(
    `${GRAPHQL_API_URL}?query=${query}&variables=${JSON.stringify(variables)}`,
    {
      headers: {
        "Prismic-Ref": previewData?.ref || prismicAPI.masterRef.ref,
        "Content-Type": "application/json",
        "Accept-Language": API_LOCALE,
        Authorization: `Token ${API_TOKEN}`,
      },
    }
  );

  if (res.status !== 200) {
    console.log(await res.text());
    throw new Error("Failed to fetch API");
  }

  const json = await res.json();
  if (json.errors) {
    console.error(json.errors);
    throw new Error("Failed to fetch API");
  }
  return json.data;
}

export async function getAllPprojectsWithSlug() {
  const data = await fetchAPI(`
    {
      allProjects {
        edges {
          node {
            _meta {
              uid
            }
          }
        }
      }
    }
  `);
  return data?.allProjects?.edges;
}

export async function getAllProjectsForHome() {
  const data = await fetchAPI(
    `
    query {
      allProjects() {
        edges {
          node {
            project_title
            project_tagline
            project_image
            project_content
            _meta {
              uid
            }
          }
        }
      }
    }
  `
  );
  return data.allProjects.edges;
}

export async function getProjectAndMoreProjectss(slug) {
  const data = await fetchAPI(
    `
  query ProjectBySlug($slug: String!, $lang: String!) {
    project(uid: $slug, lang: $lang) {
      project_title
      project_tagline
      project_content
      project_image
      _meta {
        uid
      }
    }
   moreProjects: allProjects(first: 3) {
      edges {
        node {
            project_title
            project_tagline
            project_content
            project_image
          _meta {
            uid
          }
        }
      }
    }
  }
  `,
    {
      variables: {
        slug,
        lang: API_LOCALE,
      },
    }
  );

  data.moreProjects = data.moreProjects.edges
    .filter(({ node }) => node._meta.uid !== slug)
    .slice(0, 2);

  return data;
}
